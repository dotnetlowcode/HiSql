using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Text.RegularExpressions;
using System.Threading.Tasks;
using static HiSql.Demo_Insert;

namespace HiSql.UnitTest
{
    public class Demo_Update
    {

        public static void Init(HiSqlClient sqlClient)
        {
            Update_Demo(sqlClient);//ok
            //Update_Demo2(sqlClient);//ok
            //Update_Demo3(sqlClient);//ok
            //Update_Demo4(sqlClient);//ok
            // Update_Demo5(sqlClient);//ok
        }

        static void Update_Demo5(HiSqlClient sqlClient)
        {
            string sql2 = sqlClient.Update("HTest01").Set(new { UName = "TEST" }).Where("SID=123456").ToSql();
            string sql = sqlClient.Update("HTest01").Set(new { UName = "TEST" }).Where(new Filter { { "SID", OperType.EQ, 123456 } }).ToSql();

            string sql3=sqlClient.Update("HTest01", new { SID = 123456, Salary = "`Salary`+1" }).Exclude("SID").Where("SID = 123456").ToSql();
        }
        static void Update_Demo4(HiSqlClient sqlClient)
        {
            sqlClient.BeginTran(System.Data.IsolationLevel.ReadUncommitted);
            int _effect= sqlClient.Update("HTest01", new { sid = 123456, UName = "tansar", Age = 25, Salary = 1999.9, Descript = "hello worl111da" }).ExecCommand();
            string _sql=sqlClient.Update("HTest01", new { sid = 123456, UName = "tansar", Age = 25, Salary = 1999.9, Descript = "hello worl111da" }).ToSql();

            var a = sqlClient.Modi("HTest01", new List<object> {
                new { SID = "1", UName = "普通用户2" ,  Salary = 11, Descript = "hisql"},
                new { SID = "2", UName = "中级用户3" ,    Salary = 11, Descript = "hisql"},
                new { SID = "3", UName = "高级用户4",   Salary = 11, Descript = "hisql" }
            }).ExecCommand();
            //sqlClient.CommitTran();
        }

        static void Update_Demo3(HiSqlClient sqlClient)
        {
            string _sql = sqlClient.Update("Hi_FieldModel", new { TabName = "HTest01", FieldName = "UName",Regex= @"^\d{3}[\w]+[^']$" }).Only("Regex").ToSql();
            sqlClient.Update("Hi_FieldModel", new { TabName = "HTest01", FieldName = "UName", Regex = @"^\d{3}[\w]+[^']$" }).Only("Regex").ExecCommand();

            sqlClient.Update("HTest01", new { SID = "0", UTYP = "U4", UName = "hisql", Age = 36, Salary = 11, Descript = "hisql" }).ExecCommand();
        }

        static void Update_Demo2(HiSqlClient sqlClient)
        {
            //string _txt = @"`UserAge`= `UserAge` + 1";
            string _txt = @"abc";
            string _reg = @"[`](?:[\s]*)(?:(?<flag>[\#]{1,2}|[\@]{1})?(?<tab>[\w]+)(?:[\.]{1}))?(?<field>[\w]+)[`]";
            List<Dictionary<string, string>> lst_dic = Tool.RegexGrps(_reg, _txt);

            if (lst_dic.Count() > 0)
            {
                Regex regex = new Regex(_reg);
                foreach (Dictionary<string, string> dic in lst_dic)
                {
                    _txt = regex.Replace(_txt, dic["field"].ToString(), 1);
                }
            }

            string _sql = sqlClient.Update("H_TEST", new { Hid = 1, UNAME = "UTYPE", UserAge= "`UserAge`+1", UNAME2 = "user123" }).Exclude("UNAME").ToSql();



        }

        static void Update_Demo(HiSqlClient sqlClient)
        {

            Dictionary<string, string> _dic = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { { "Hid", "1" }, { "UNAME", "user123" } };


            List<object> lstdyn = new List<object>();
            for (int i = 0; i < 10; i++)
            {
                TDynamic dyn1 = new TDynamic();
                dyn1["Hid"] = 150 + i;
                dyn1["UserName"] = $"tgm{i}";
                dyn1["UserAge"] = 34;
                dyn1["Birth"] = DateTime.Now;
                lstdyn.Add(dyn1);
            }

            IUpdate _dicupdate = sqlClient.Update("H_TEST", lstdyn);
            string _dicsql = _dicupdate.ToSql();


            IUpdate update = sqlClient.Update("H_TEST", new H_Test { Hid = 1, UserName = "UTYPE", UserAge=22 }).Exclude("UNAME");//,

            //int _effect = sqlClient.Update("H_TEST", new { Hid = 1, UNAME = "UTYPE", UNAME2 = "user123" }).Exclude("UNAME").ExecCommand();
            string _sql = update.ToSql();
            Console.WriteLine(_sql);


            IUpdate update1 = sqlClient.Update("H_TEST").Set(new { UserName = "TEST" }).Where(new Filter { { "Hid", OperType.GT, 8 } });
            IUpdate update1_2 = sqlClient.Update("H_TEST").Set(new { UserName = "TEST" }).Where("Hid>8");
            //int _effect1 = sqlClient.Update("H_TEST").Set(new { UserName = "TEST" }).Where(new Filter { { "Hid", OperType.GT, 8 } }).ExecCommand();
            string _sql1 = update1.ToSql();
            string _sql12 = update1_2.ToSql();
            Console.WriteLine(_sql1);

            IUpdate update2 = sqlClient.Update<H_Test>(new H_Test { Hid = 1 ,UserName="Haha1"}).Only("UserName");
            //int _effect2 = sqlClient.Update<H_Test>(new H_Test { Hid = 1, UserName = "Haha1" }).Only("UNAME2").ExecCommand();
            string _sql2 = update2.ToSql();
            Console.WriteLine(_sql2);

             
            IUpdate update3 = sqlClient.Update("H_TEST", new List<object> { new { Hid = 1, UserName = "user123" }, new { Hid = 2, UserName = "user124" } }).Only("UserName");
            //int _effect3 = sqlClient.Update("H_TEST", new List<object> { new { Hid = 1, UserName = "user123" }, new { Hid = 2, UserName = "user124" } }).Only("UserName").ExecCommand();
            string _sql3 = update3.ToSql();
            Console.WriteLine(_sql3);

            //"did > 8"



            if (string.IsNullOrEmpty(_sql2))
            { 
                
            }

        }
    }
}
