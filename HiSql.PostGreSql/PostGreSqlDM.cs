...
namespace HiSql
{
    public class PostGreSqlDM : IDM
    {
        StringBuilder _Sql = new StringBuilder();
        public virtual HiSqlProvider Context { get; set; }
        PostGreSqlConfig dbConfig = new PostGreSqlConfig(true);
        public PostGreSqlDM()
        {
        }
        ...
        public DBVersion DBVersion()
        {
            ...
            var version = CacheContext.LocalMCahe.GetOrCreate(typeof(PostGreSqlDM).FullName + "_DBVersion", () =>
            {
        ...
        {
            var table = Context.DBO.GetDataTable("select now();");
        ...
        public string GetDbDefault(HiColumn hiColumn, string tabname = "")
        {
            ...
            if (hiColumn.DBDefault != HiTypeDBDefault.NONE)
            {
                if (hiColumn.FieldType.IsIn<HiType>(HiType.NCHAR, HiType.CHAR, HiType.NVARCHAR, HiType.VARCHAR))
                    _default = "''";
                else if (hiColumn.FieldType.IsIn<HiType>(HiType.INT, HiType.BIGINT, HiType.SMALLINT, HiType.DECIMAL))
                {
                ...
                {
                    if (hiColumn.DefaultValue.Trim().ToLower().Trim() == "false")
                        _default = "False";
                    else if (hiColumn.DefaultValue.Trim().ToLower().Trim() == "true")
                        _default = "True";
                    else
                        _default = "False";
                }
            ...
            if (hiColumn.IsIdentity)
                return $"DEFAULT nextval('\"{tabname}_{hiColumn.FieldName}_seq\"'::regclass)";
            return _default;
        }
                ...
                newtabinfo = HiSqlCommProvider.InitTabMaping(_keyname, () =>
                {
                    ...
                    else
                    {
                            ...
                            )
                        {
                            ...
                            var phytabInfo = TabDefinitionToEntity(dts, dbConfig.DbMapping);
                            List<FieldChange> fieldChanges = HiSqlCommProvider.TabToCompare(phytabInfo, tabInfo,DBType.PostGreSql);
        ...
        string inertTabStruct(HiTable hiTable, List<HiColumn> lstHiTable)
        {
            ...
            sb.AppendLine($"insert into  {dbConfig.Schema_Pre}{_schema}{dbConfig.Schema_After}.{dbConfig.Table_Pre}{Constants.HiSysTable["Hi_TabModel"]}{dbConfig.Table_After} (")
               ...
               .Append($"{dbConfig.Field_Pre}DbName{dbConfig.Field_After}{dbConfig.Field_Split}")
                ...
                .Append(hiTable.IsSys == true ? "True" : "False")
                .Append(',')
                .Append(hiTable.IsEdit == true ? "True" : "False")
                .Append(',')
            ...
            {
                _dbname = string.IsNullOrEmpty(hiColumn.DbName) ? "" : hiColumn.DbName;
                ...
                sb.AppendLine($"insert into  {dbConfig.Schema_Pre}{_schema}{dbConfig.Schema_After}.{dbConfig.Table_Pre}{Constants.HiSysTable["Hi_FieldModel"]}{dbConfig.Table_After} (")
                    ...
                    .Append(hiColumn.IsIdentity == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsPrimary == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsBllKey == true ? "True" : "False")
                    .Append(',')
                    ...
                    .Append(hiColumn.IsSys == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsNull == true ? "True" : "False") //postgresql is not null
                    .Append(',')
                    .Append(hiColumn.IsRequire == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsIgnore == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsObsolete == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsShow == true ? "True" : "False")
                    .Append(',')
                    .Append(hiColumn.IsSearch == true ? "True" : "False")
                    .Append(',')
                    .Append($"'{(int)hiColumn.SrchMode}',")
                    .Append(hiColumn.IsRefTab == true ? "True" : "False")
                    .Append(',')
        ...
        /// <summary>
        /// 由于PostGreSql 实现批量更新插入时与其它数据库逻辑一样，帮做特殊处理
        /// </summary>
        /// <param name=""></param>
        /// <returns></returns>
        public string BuildMergeIntoSqlSequence(TabInfo targetinfo, List<string> dataColLst = null)
        {
            ...
            {
                _filer.Add($"{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}");
            }
            if (_filer.Count == 0)
                throw new Exception($"表[{targetinfo.TabModel.TabName}]无主键");

            _filterstr = string.Join(dbConfig.Field_Split, _filer);
            ...
            if (_lstupdate.Count == 0)
                throw new Exception($"表[{targetinfo.TabModel.TabName}]无可更新或插入的字段");

            return _merge_temp.Replace("[$Keys$]", _filterstr)
        ...
        public string BuildChangeFieldStatement(HiTable hiTable, HiColumn hiColumn, TabFieldAction tabFieldAction)
        {
            ...
            {
                _fieldsql = BuildFieldStatementForModi(hiTable, hiColumn);
            }
            ...
            if (tabFieldAction == TabFieldAction.ADD)
                _changesql = dbConfig.Add_Column.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$TempColumn$]", _fieldsql);
            else if (tabFieldAction == TabFieldAction.DELETE)
                _changesql = dbConfig.Del_Column.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$FieldName$]", hiColumn.FieldName);
            else if (tabFieldAction == TabFieldAction.MODI)
            {
                //修改字段类型
                _changesql = dbConfig.Modi_Column.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$TempColumn$]", _fieldsql);
                //设置默认值
                if (hiColumn.DefaultValue == null || hiColumn.DefaultValue.IsNullOrEmpty())
                {
                    _changesql += dbConfig.Del_Default.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$FieldName$]", hiColumn.FieldName);
                }
                else
                {
                    if (hiColumn.IsIdentity)
                    {
                        string _sequence_str = buildSequence(hiTable.TabName, new List<HiColumn> { hiColumn }, dbConfig.Sequence_Temp);
                        _changesql += _sequence_str;
                    }

                    _changesql += dbConfig.Set_Default.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$FieldName$]", hiColumn.FieldName).Replace("[$DEFAULT$]", GetDbDefault(hiColumn, hiTable.TabName));
                }
                //设置是否为空约束
                if (hiColumn.IsNull)
                {
                    _changesql += dbConfig.Del_NotNull.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$FieldName$]", hiColumn.FieldName);
                }
                else
                {
                    _changesql += dbConfig.Set_NotNull.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$FieldName$]", hiColumn.FieldName);
                }

                if (!hiColumn.FieldDesc.IsNullOrEmpty())
                {
                    _changesql += dbConfig.Field_Comment.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$FieldName$]", hiColumn.FieldName).Replace("[$FieldDesc$]", hiColumn.FieldDesc);
                }
            ...
            {
                _changesql = dbConfig.Re_Column.Replace("[$TabName$]", hiTable.TabName).Replace("[$Schema$]", _schema).Replace("[$ReFieldName$]", hiColumn.ReFieldName).Replace("[$FieldName$]", hiColumn.FieldName);
                ...
                return _changesql + _changesqlUpdate;
            }
        ...
        public string BuildFieldStatement(HiTable hiTable, HiColumn hiColumn, bool isalteraddkey = false)
        {
            ...
            if (dbConfig.DbMapping.ContainsKey(hiColumn.FieldType))
            {
                if (dbConfig.FieldTempMapping.ContainsKey(dbConfig.DbMapping[hiColumn.FieldType].ToString()))
                {
                    ...
                    string _postgre_identity = string.Empty;

                    switch (dbConfig.DbMapping[hiColumn.FieldType].ToString().ToLower())
                    {
                        case "nvarchar":
                        case "varchar":
                        case "nchar":
                        ...
                        case "bigint":
                            if (hiColumn.IsIdentity)
                            {
                                _postgre_identity = $" DEFAULT nextval('\"{hiTable.TabReName}_{hiColumn.FieldName}_seq\"'::regclass)";
                            }
                            _str_temp_field = _str_temp_field.Replace("[$FieldName$]", hiColumn.FieldName)
                                .Replace("[$IsIdentity$]", hiColumn.IsIdentity ? _postgre_identity : "")
        ...
        public string BuildFieldStatementForModi(HiTable hiTable, HiColumn hiColumn)
        {
            string _str_temp_field = "";
            if (dbConfig.DbMapping.ContainsKey(hiColumn.FieldType))
            {
                if (dbConfig.FieldTempMappingForModi.ContainsKey(dbConfig.DbMapping[hiColumn.FieldType].ToString()))
                {
                    _str_temp_field = dbConfig.FieldTempMappingForModi[dbConfig.DbMapping[hiColumn.FieldType].ToString()].ToString();
        ...
        public DataTable GetTableDefinition(string tabname)
        {
            ...
            DataTable dt = Context.DBO.GetDataTable(
                dbConfig.Get_Table_Schema.Replace("[$TabName$]", tabname)
                .Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema)
                );
            if (dt.Rows.Count == 0)
                throw new Exception($"表[{tabname}]不存在");
        ...
        private string buildSequence(string tabname, List<HiColumn> lstHiTable, string sequence_temp)
        {
            StringBuilder sb = new StringBuilder();


            var lstcolumn = lstHiTable.Where(c => c.IsIdentity).ToList();
        ...
        public string BuildTabCreateSql(HiTable hiTable, List<HiColumn> lstHiTable, bool isdrop = false)
        {
            ...
            string _table_keys = dbConfig.Table_Key3;
            string _table_seqeuence = dbConfig.Sequence;
            if (hiTable != null)
            {
                if (hiTable.TableType == TableType.Var)  // && !hiTable.IsGlobalTemp && !hiTable.IsLocalTemp
                {
                    ...
                    _table_keys = dbConfig.Table_Key3_Temp;
                    _table_seqeuence = dbConfig.Sequence_Temp;
                }
        ...
        public string BuildFieldStatment(HiTable hiTable, List<HiColumn> lstColumn)
        {
            ...
            foreach (HiColumn hiColumn in lstColumn)
            {
                ...
                _lst.Add(BuildFieldStatement(hiTable, hiColumn) + Environment.NewLine);
            }

            sb_sql.AppendLine(string.Join(dbConfig.Field_Split, _lst));
        ...
        /// <returns></returns>
        public string BuilderWhereSql(List<TableDefinition> TableList, Dictionary<string, TabInfo> dictabinfo, List<FieldDefinition> Fields, List<WhereResult> lstresult, bool issubquery, bool hastabname=true)
        {
            StringBuilder sb_sql = new StringBuilder();
                ...
                foreach (WhereResult whereResult in lstresult)
                {
                        ...
                        if (whereResult.Result.ContainsKey("fields") && whereResult.Result.ContainsKey("rfields"))
                        {
                            ...
                            if (hiColumn != null && rhiColumn != null)
                            {
                                ...
                                if (hastabname)
                                    ...
                                    sb_sql.Append($"{dbConfig.Field_Pre}{Tool.GetDbFieldName(rhiColumn, rfield)}{dbConfig.Field_After}");
                            }
                        }
                        else
                    ...
                    else if (whereResult.SType == StatementType.SubCondition)
                    {
                        ...
                        if (whereResult.Result != null)
                        {
                            ...
                            sb_sql.Append($" ({BuilderWhereSql(TableList, dictabinfo, Fields, whereParse.Result, issubquery, hastabname)})");
                        }
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public string BuilderWhereSql(List<TableDefinition> TableList, Dictionary<string, TabInfo> dictabinfo, List<FieldDefinition> Fields, List<FilterDefinition> Wheres, bool issubquery, bool hastabname=true)
        {
            ...
            foreach (FilterDefinition filterDefinition in Wheres)
            {
                ...
                else if (filterDefinition.FilterType == FilterType.CONDITION)
                {
                    ...
                    if (hastabname)
                        ...
                        sb_where.Append($"{dbConfig.Field_Pre}{Tool.GetDbFieldName(hiColumn, filterDefinition.Field)}{dbConfig.Field_After}");
                        ...
                        if (hiColumn != null)
                        {
                            switch (fieldDefinition.DbFun)
                            {
                                case DbFunction.AVG:
                                    ...
                                    {
                                        sb_field.Append($"avg({dbConfig.Table_Pre}{Tool.GetDbTabName(hiColumn, fieldDefinition)}{dbConfig.Table_After}.{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}) as {dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
                                ...
                                case DbFunction.SUM:
                                    if (hiColumn.FieldType.IsIn<HiType>(HiType.BIGINT, HiType.DECIMAL, HiType.INT, HiType.SMALLINT))
                                    {
                                        sb_field.Append($"sum({dbConfig.Table_Pre}{Tool.GetDbTabName(hiColumn, fieldDefinition)}{dbConfig.Table_After}.{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}) as {dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
                                        sb_field_result.Append($"{dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
                                ...
                                case DbFunction.MAX:
                                    if (hiColumn.FieldType.IsIn<HiType>(HiType.BIGINT, HiType.DECIMAL, HiType.INT, HiType.SMALLINT, HiType.NVARCHAR, HiType.VARCHAR, HiType.NCHAR, HiType.CHAR, HiType.DATE,
                                        HiType.DATETIME))
                                    {
                                        sb_field.Append($"max({dbConfig.Table_Pre}{Tool.GetDbTabName(hiColumn, fieldDefinition)}{dbConfig.Table_After}.{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}) as {dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
                                        sb_field_result.Append($"{dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
                                ...
                                case DbFunction.MIN:
                                    if (hiColumn.FieldType.IsIn<HiType>(HiType.BIGINT, HiType.DECIMAL, HiType.INT, HiType.SMALLINT, HiType.NVARCHAR, HiType.VARCHAR, HiType.NCHAR, HiType.CHAR, HiType.DATE,
                                        HiType.DATETIME))
                                    {
                                        sb_field.Append($"min({dbConfig.Table_Pre}{Tool.GetDbTabName(hiColumn, fieldDefinition)}{dbConfig.Table_After}.{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}) as {dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
                                        sb_field_result.Append($"{dbConfig.Field_Pre}{fieldDefinition.AsFieldName}{dbConfig.Field_After}");
        ...
        string getSingleValue(bool issubquery, HiColumn hiColumn, object value)
        {
            ...
            if (!issubquery)
            {
                ...
                {
                    if (string.Equals("0", value.ToString(), StringComparison.OrdinalIgnoreCase))
                    {
                        return "false";
                    }
        ...
        string getSingleValue(bool issubquery, HiColumn hiColumn, FilterDefinition filterDefinition, object value, List<TableDefinition> TableList = null, Dictionary<string, TabInfo> dictabinfo = null)
        {
            ...
            if (!issubquery)
            {
                ...
                if (hiColumn.FieldType.IsIn<HiType>(HiType.NCHAR, HiType.NVARCHAR, HiType.GUID))
                {
                    ...
                    if (!_istempfield)
                    {
                        if (_value.Length <= hiColumn.FieldLen + _likesymbol)
                        {
                            if (hiColumn.IsPrimary && string.IsNullOrEmpty(_value))
                            {
                                _value = $"'{dbConfig.Key_Char_Default}'";
                ...
                else if (hiColumn.FieldType.IsIn<HiType>(HiType.VARCHAR, HiType.CHAR))
                {
                    ...
                    if (!_istempfield)
                    {
                        if (_value.LengthZH() <= hiColumn.FieldLen + _likesymbol)
                        {
                            if (hiColumn.IsPrimary && string.IsNullOrEmpty(_value))
                            {
                                _value = $"'{dbConfig.Key_Char_Default}'";
        ...
        public DataTable GetTableList(string tabname = "")
        {
            ...
            if (string.IsNullOrEmpty(tabname))
                ...
                _tempsql = _tempsql.Replace("[$Where$]", $" and tablename='{tabname.ToSqlInject()}'");
        ...
        public DataTable GetTableList(string tabname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            string _tempsql = dbConfig.Get_TablesPaging.Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema);

            if (!string.IsNullOrEmpty(tabname))
                _where += $" and tablename like '%{tabname.ToSqlInject()}%'";
        ...
        public DataTable GetViewList(string viewname = "")
        {
            ...
            if (string.IsNullOrEmpty(viewname))
                ...
                _tempsql = _tempsql.Replace("[$Where$]", $" and viewname='{viewname.ToSqlInject()}'");
            return Context.DBO.GetDataTable(_tempsql);
            throw new NotImplementedException();
        }
        public DataTable GetViewList(string viewname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            int SeqEnd = (pageIndex) * pageSize;
            var _where = string.Empty;
            string _tempsql = dbConfig.Get_ViewsPaging.Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema);

            if (!string.IsNullOrEmpty(viewname))
                _where += $" and viewname like '%{viewname.ToSqlInject()}%'";
        ...
        public DataTable GetAllTables(string tabname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            int SeqEnd = (pageIndex) * pageSize;
            var _where = string.Empty;
            string _tempsql = dbConfig.Get_AllTablesPaging.Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema);

            if (!string.IsNullOrEmpty(tabname))
                _where += $@" and ""TabName"" like '%{tabname.ToSqlInject()}%'";
        ...
        {
            string _sql = dbConfig.Re_Table.Replace("[$TabName$]", $"{tabname}").Replace("[$ReTabName$]", $"{newtabname}").Replace("[$Schema$]", this.Context.CurrentConnectionConfig.Schema); ;
            return _sql;
        }
