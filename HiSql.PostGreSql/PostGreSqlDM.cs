    ...
    public class PostGreSqlDM : IDM
    {
        ...
        public string GetDbDefault(HiColumn hiColumn,string tabname="")
        {
            ...
            if (hiColumn.IsIdentity)
                return $"DEFAULT nextval('\"{tabname}_{hiColumn.ColumnName}_seq\"'::regclass)";
            return _default;
        }
        /// <summary>
        /// 获取表结构信息并缓存
        ...
        public string BuildMergeIntoSqlSequence(TabInfo targetinfo)
        {
            ...
            {
                _filer.Add($"{dbConfig.Field_Pre}{hiColumn.ColumnName}{dbConfig.Field_After}");
            }
            if (_filer.Count == 0)
            ...
            foreach (HiColumn hiColumn in targetinfo.GetColumns)
            {
                ...
                {
                    _lstupdate.Add($"{dbConfig.Field_Pre}{hiColumn.ColumnName}{dbConfig.Field_After}=excluded.{dbConfig.Field_Pre}{hiColumn.ColumnName}{dbConfig.Field_After}");
                }
                ...
                if (dbConfig.FieldTempMapping.ContainsKey(dbConfig.DbMapping[hiColumn.FieldType].ToString()))
                {
                    ...
                    switch (dbConfig.DbMapping[hiColumn.FieldType].ToString().ToLower())
                    {
                        ...
                        case "bigint":
                            if (hiColumn.IsIdentity)
                            {
                                _postgre_identity = $" DEFAULT nextval('\"{hiTable.TabReName}_{hiColumn.ColumnName}_seq\"'::regclass)";
                            }
                            _str_temp_field = _str_temp_field.Replace("[$ColumnName$]", hiColumn.ColumnName)
