...
;
            ...
            */
            ...
            _temp_get_table_schema = new StringBuilder()
                ...
                .AppendLine("   case when exists(SELECT kcu.column_name FROM information_schema.table_constraints AS tc JOIN information_schema.key_column_usage AS kcu  ON tc.constraint_name = kcu.constraint_name AND tc.table_schema = kcu.table_schema AND tc.table_name = kcu.table_name WHERE  tc.constraint_type = 'PRIMARY KEY'  	   AND  tc.table_name = '[$TabName$]' AND tc.table_schema = '[$Schema$]' and kcu.column_name= A.attname ) then 'True' else 'False' end as \"IsPrimary\",")
                .AppendLine("   case when   A .attnotnull = 't' and (select count(column_default) from information_schema.columns where table_schema='[$Schema$]' and  table_name = '[$TabName$]' and column_name =A .attname and column_default like 'nextval(%' )='1'  and (select count(pc.conname) from pg_constraint  pc where a.attnum = ANY(pc.conkey) and pc.conrelid = c.oid) = '1' then 'True' else 'Fasle' end as \"IsIdentity\"")
                .AppendLine("FROM")
...
SET "DomainDesc" = excluded."DomainDesc";
            ...
            //删除临时表 add by tgm date:2025.2.14
            _temp_drop_tmp_table = $"drop table {_temp_table_pre}[$TabName$]{_temp_table_after};";
