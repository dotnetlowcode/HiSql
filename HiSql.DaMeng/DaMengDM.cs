...
namespace HiSql
{
    public class DaMengDM : IDM
    {
        StringBuilder _Sql = new StringBuilder();
        public virtual HiSqlProvider Context { get; set; }
        DaMengConfig dbConfig = new DaMengConfig(true);
        public DaMengDM()
        {
        }
        ...
        {
            var version = CacheContext.LocalMCahe.GetOrCreate(typeof(DaMengDM).FullName + "_DBVersion", () =>
            {
        ...
        public TabInfo TabDefinitionToEntity(DataTable table, Dictionary<HiType, string> _dbmapping)
        {
                ...
                )
            {
                ...
                foreach (DataRow drow in table.Rows)
                {
                    ...
                    else if (_dbdefault.ToLower() == "SYSTIMESTAMP".ToLower())
                    {
        ...
        public string GetDbDefault(HiColumn hiColumn, string tabname = "")
        {
            ...
            if (hiColumn.IsIdentity)
                return $"default \"{tabname}_{hiColumn.FieldName}_SEQ\".\"NEXTVAL\"";
            ...
            if (newtabinfo == null)
            {
                newtabinfo = HiSqlCommProvider.InitTabMaping(_keyname, () =>
                {
                    ...
                    if (tabInfo == null)
                    {
                        ...
                        {
                            //外层有锁 此处可以不加锁 add by tgm date:2024.6.18
                        ...
                        else
                        {
                            ...
                            if (Constants.HiSysTable["Hi_TabModel"].Equals(tabInfo.TabModel.TabName, StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                tabInfo = dmNvarchar(tabInfo, _tabinfo);
                            }
                    ...
                    else
                    {
                            ...
                            )
                        {
                            ...
                            var phytabInfo = TabDefinitionToEntity(dts, dbConfig.DbMapping);
                            List<FieldChange> fieldChanges = HiSqlCommProvider.TabToCompare(phytabInfo, tabInfo,DBType.DaMeng);
                            //fieldChanges=dmFieldChange(fieldChanges);
        ...
        List<FieldChange> dmFieldChange(List<FieldChange> fieldChanges)
        {
            List < FieldChange > _fields=new List<FieldChange> ();
            foreach (FieldChange fieldChange in fieldChanges) {
                if (fieldChange.Action == TabFieldAction.MODI)
                {
                    FieldChangeDetail fieldtype= fieldChange.ChangeDetail.Where(c => c.AttrName.Equals("FieldType", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();

                    FieldChangeDetail fieldlen = fieldChange.ChangeDetail.Where(c => c.AttrName.Equals("FieldLen", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                    if (fieldtype != null && fieldlen != null)
                    {
                        if (fieldtype.ValueA.Equals("varchar", StringComparison.OrdinalIgnoreCase) && fieldtype.ValueB.Equals("nvarchar", StringComparison.OrdinalIgnoreCase))
                        {
                            int a = int.Parse( fieldlen.ValueA);
                            int b=int.Parse(fieldlen.ValueB);
                            if (a % 2 == 0 && a / 2 == b)
                            { 
                                //remove
                            }
                            else
                                _fields.Add(fieldChange);
        ...
        /// <summary>
        /// 由于达梦数据库不支持nvarchar和nchar 在进行转换时需要进行特殊处理
        /// </summary>
        /// <param name="tabinfo"></param>
        /// <param name="eftabinfo"></param>
        /// <returns></returns>
        TabInfo dmNvarchar(TabInfo tabinfo, TabInfo eftabinfo)
        { 
            HiColumn[] columns=eftabinfo.Columns.Where(col=>col.FieldType==HiType.NVARCHAR || col.FieldType==HiType.NCHAR).ToArray();

            foreach (HiColumn col in columns)
            { 
                HiColumn column=tabinfo.Columns.Where(c=>c.FieldName.Equals( col.FieldName,StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                if (column != null)
                {
                    column.FieldLen = col.FieldLen;// col.FieldLen % 2 == 0 ? col.FieldLen / 2 : col.FieldLen / 2 + 1;
        ...
        /// <returns></returns>
        string inertTabStruct(HiTable hiTable, List<HiColumn> lstHiTable, bool isimimmediate = false)
        {
            ...
            if (isimimmediate)
            {
                _insertlst.Add($"execute immediate '{sb.ToString().Replace("'", "''")}';");
            }
            else _insertlst.Add($"{sb.ToString()};");
            ...
            return string.Join(Environment.NewLine, _insertlst);
        }
        /// <summary>
        ...
        public string BuildInsertSql(TableType tableType,Dictionary<string, string> _values, bool isbulk = false)
        {
            ...
            StringBuilder _sb_field = new StringBuilder();
            StringBuilder _sb_value = new StringBuilder();
            string _schema = string.IsNullOrEmpty(Context.CurrentConnectionConfig.Schema) ? $" {dbConfig.Schema_Pre}[$Schema$]{dbConfig.Schema_After}." : "";
            ...
            {
                //return $"into  {_schema}{dbConfig.Table_Pre}[$TabName$]{dbConfig.Table_After}({_sb_field.ToString()}) values ({_sb_value.ToString()})";
                return $" UNION ALL select {_sb_value.ToString()} from dual";
            }
        }
        ...
        public string BuildMergeIntoSql(TabInfo targetinfo, TabInfo sourceinfo, List<string> dataColLst = null, List<string> keyColList = null)
        {
            ...
            _merge_temp = _merge_temp
                ...
                .Replace("[$Source$]", sourceinfo.TabModel.TabReName)
        ...
        public string BuildChangeFieldStatement(HiTable hiTable, HiColumn hiColumn, TabFieldAction tabFieldAction)
        {
            ...
            {
                _changesql.AppendLine($"    execute immediate '{dbConfig.Add_Column.Replace("[$TabName$]", $"{dbConfig.Table_Pre}{hiTable.TabName}{dbConfig.Table_After}").Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema).Replace("[$TempColumn$]", _fieldsql).ToString().Replace("'", "''")}';");
            ...
            {
                _changesql.AppendLine($"    execute immediate '{dbConfig.Del_Column.Replace("[$TabName$]", $"{dbConfig.Table_Pre}{hiTable.TabName}{dbConfig.Table_After}").Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema).Replace("[$FieldName$]", $"{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}").ToString().Replace("'", "''")}';");

            }

            else if (tabFieldAction == TabFieldAction.MODI)
            {
                _changesql.AppendLine($"    execute immediate '{dbConfig.Modi_Column.Replace("[$TabName$]", $"{dbConfig.Table_Pre}{hiTable.TabName}{dbConfig.Table_After}").Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema).Replace("[$TempColumn$]", _fieldsql).ToString().Replace("'", "''")}';");
            ...
            if (dbConfig.DbMapping.ContainsKey(hiColumn.FieldType))
            {
                if (dbConfig.FieldTempMapping.ContainsKey(dbConfig.DbMapping[hiColumn.FieldType].ToString()))
                {
                    ...
                    switch (dbConfig.DbMapping[hiColumn.FieldType].ToString())
                    {
                        ...
                        case "smallint":
                            _str_temp_field = _str_temp_field.Replace("[$FieldName$]", hiColumn.FieldName)
                                .Replace("[$IsNull$]", hiColumn.IsPrimary ? "" : hiColumn.IsNull == true ? hiColumn.DBDefault != HiTypeDBDefault.NONE ? "" : "NULL" : "NOT NULL")
                                .Replace("[$EXTEND$]", hiTable.TableType == TableType.Var && hiColumn.IsPrimary ? "primary key" : "")
        ...
        public string BuildTabStructSqlv2(HiTable hiTable, List<HiColumn> lstHiTable)
        {
            return inertTabStruct(hiTable, lstHiTable, true);
        }
            ...
            if (hiTable != null)
            {
                ...
                else if (hiTable.TableType == TableType.Local)
                {
                    ...
                    if (!isdrop)
                        ...
                        _temp_create = dbConfig.Table_Local_Create_Drop;
                ...
                _temp_create = _temp_create.Replace("[$Schema$]", string.IsNullOrEmpty(hiTable.Schema) ? Context.CurrentConnectionConfig.Schema : hiTable.Schema)
                    ...
                    .Replace("[$Primary$]", hiTable.TableType == TableType.Var ? "" : keys);
                    ...
                    )
                {
                    ...
                    foreach (HiColumn hiColumn in lstHiTable)
                    {
                        ...
                        _comment = $"execute immediate '{_comment.Replace("'", "''")}';";
        ...
        public string BuildFieldStatment(HiTable hiTable, List<HiColumn> lstColumn)
        {
            ...
            {
                //仅oracle 采用此方式
                if (i < lstColumn.Count)
                    sb_sql.AppendLine($" || '{BuildFieldStatement(hiTable, hiColumn).Replace("'", "''")},'");
                else
                    sb_sql.AppendLine($" || '{BuildFieldStatement(hiTable, hiColumn).Replace("'", "''")}'");
                i++;
            }
        ...
        public DataTable GetTableList(string tabname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            string _tempsqlcount = dbConfig.Get_TablesPagingCount.Replace("[$Schema$]", this.Context.CurrentConnectionConfig.Schema);
            ...
            _tempsqlcount = _tempsqlcount.Replace("[$Where$]", _where);
        ...
        public DataTable GetViewList(string viewname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            int SeqEnd = (pageIndex) * pageSize;
            var _where = string.Empty;
            string _tempsqlcount = dbConfig.Get_ViewsPagingCount.Replace("[$Schema$]", this.Context.CurrentConnectionConfig.Schema);


            string _tempsql = dbConfig.Get_ViewsPaging.Replace("[$Schema$]", this.Context.CurrentConnectionConfig.Schema);

            if (!string.IsNullOrEmpty(viewname))
                _where += $@" and VIEW_NAME like '%{viewname.ToSqlInject()}%'";
        ...
        public DataTable GetAllTables(string tabname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            int SeqEnd = (pageIndex) * pageSize;
            var _where = string.Empty;
            string _tempsqlcount = dbConfig.Get_AllTablesPagingCount.Replace("[$Schema$]", this.Context.CurrentConnectionConfig.Schema);


            string _tempsql = dbConfig.Get_AllTablesPaging.Replace("[$Schema$]", this.Context.CurrentConnectionConfig.Schema);
            ...
            if (dt.Rows.Count > 0)
            {
                if (hiColumns.Count > 0)
                {
                    ...
                    foreach (HiColumn hiColumn in hiColumns)
                    {
                        ...
                        string _tempkey = dbConfig.Table_Key2.Replace("[$FieldName$]", $"{dbConfig.Field_Pre}{hiColumn.FieldName}{dbConfig.Field_After}").Replace("[$Schema$]", Context.CurrentConnectionConfig.Schema);
