...
namespace HiSql.MySqlUnitTest
{
    public class Demo_Update
    {
        ...
        static void Update_Demo(HiSqlClient sqlClient)
        {
            ...
            //IUpdate update4 = sqlClient.Update("H_TEST").Set(new { UNAME2 = "`a.name` ='tgm' and a.age in ('aa','ddd') " }).Where(new Filter { { "DID", OperType.GT, 8 } });
            //UPDATE H_TEST SET UNAME = UNAME2 WHERE DID > 5;
