...
using static HiSql.DataConvert;
...
namespace HiSql
{
    ...
    /// 数据库的基础操作类
    /// </summary>
    public partial class SqliteDM : IDM
    {
        StringBuilder _Sql = new StringBuilder();
        public virtual HiSqlProvider Context { get; set; }
        SqliteConfig dbConfig = new SqliteConfig(true);
        public SqliteDM()
        {
        }
        MCache mcache = new MCache(typeof(SqliteDM).FullName);
        ...
        public TabInfo TabDefinitionToEntity(DataTable table, Dictionary<HiType, string> _dbmapping)
        {
                ...
                )
            {
                ...
                foreach (DataRow drow in table.Rows)
                {
                        ...
                        if (def.IsRegex)
                        {
                            ...
                            if (_dic.Count > 0 && _dic.ContainsKey("value"))
                            {
                                ...
                                if (hiColumn.FieldType.IsIn<HiType>(HiType.GUID))
                                {
                                    ...
                                    hiColumn.DefaultValue = Constants.FunGuid;
                                }
                    ...
                    //}
                    //else if (_dbdefault.ToLower() == "getdate()".ToLower())
                    //{
                    //    hiColumn.DBDefault = HiTypeDBDefault.FUNDATE;
                    //}
            ...
            if (hiColumn.DBDefault != HiTypeDBDefault.NONE)
            {
                ...
                else if (hiColumn.FieldType.IsIn<HiType>(HiType.DATE, HiType.DATETIME))
                {
                    ...
                    if (hiColumn.DefaultValue.Trim().ToLower() == Constants.FunDate.ToLower().Trim())
                        _default = "datetime('now', 'localtime')";
                    else
                        _default = "datetime('now', 'localtime')";
                }
                else _default = "";
        ...
        public TabInfo GetTabStruct(string tabname)
        {
            ...
            return DataConvert.CloneTabInfo(newtabinfo);
        ...
        string DeleteTabStruct(HiTable hiTable)
        {
            ...
            //sb.AppendLine($"delete from {dbConfig.Table_Pre}{Constants.HiSysTable["Hi_TabModel"].ToString()}{dbConfig.Table_After} where TabName='{hiTable.TabName}';");
            //sb.AppendLine($"delete from {dbConfig.Table_Pre}{Constants.HiSysTable["Hi_FieldModel"].ToString()}{dbConfig.Table_After} where TabName='{hiTable.TabName}';");
            return dbConfig.Delete_TabStruct.Replace("[$TabName$]", hiTable.TabName);
        }
        ...
        public string BuildChangeFieldStatement(HiTable hiTable, HiColumn hiColumn, TabFieldAction tabFieldAction)
        {
            ...
            else if ((tabFieldAction == TabFieldAction.DELETE || tabFieldAction == TabFieldAction.MODI || tabFieldAction == TabFieldAction.RENAME)
                ...
                )
            {
                ...
                _sql.AppendLine(dbConfig.Re_Table.Replace("[$ReTabName$]", $"{temp_table}"));
                var tabInfo = GetTabStruct(hiTable.TabName);
                TabInfo _tabcopy = ClassExtensions.DeepCopy<TabInfo>(tabInfo);
                if (!_tabcopy.Columns.Any(t => t.FieldName == hiColumn.FieldName))
                {
                    return "";
                }
                string selectFieldsStr = string.Empty;
        ...
        public string BuildFieldStatement(HiTable hiTable, HiColumn hiColumn, bool isalteraddkey = false)
        {
            ...
            if (dbConfig.DbMapping.ContainsKey(hiColumn.FieldType))
            {
                if (dbConfig.FieldTempMapping.ContainsKey(dbConfig.DbMapping[hiColumn.FieldType].ToString()))
                {
                    ...
                    switch (dbConfig.DbMapping[hiColumn.FieldType].ToString())
                    {
                        ...
                        case "nvarchar":
                            _str_temp_field = _str_temp_field.Replace("[$FieldName$]", hiColumn.FieldName)
                                .Replace("[$FieldLen$]", hiColumn.FieldLen < 0 ? "4000" : hiColumn.FieldLen.ToString())
                        ...
                        case "bigint":
                            _str_temp_field = _str_temp_field.Replace("[$FieldName$]", hiColumn.FieldName)
                                ...
                                .Replace("[$IsNull$]", hiColumn.IsPrimary ? "NOT NULL" : hiColumn.IsIdentity ? "NOT NULL" : hiColumn.IsNull == false ? "NULL" : "NOT NULL")
                        ...
                        case "smallint":
                            _str_temp_field = _str_temp_field.Replace("[$FieldName$]", hiColumn.FieldName)
                                .Replace("[$IsNull$]", hiColumn.IsPrimary ? "" : hiColumn.IsNull == false ? "NULL" : "NOT NULL")
                                .Replace("[$EXTEND$]", hiTable.TableType == TableType.Var && hiColumn.IsPrimary ? "primary key" : "")
                        ...
                        case "decimal":
                            _str_temp_field = _str_temp_field.Replace("[$FieldName$]", hiColumn.FieldName)
                                ...
                                .Replace("[$IsNull$]", hiColumn.IsIdentity ? "NOT NULL" : hiColumn.IsNull == false ? "NULL" : "NOT NULL")
                                .Replace("[$Default$]", hiColumn.IsPrimary || hiColumn.IsIdentity ? "" : GetDbDefault(hiColumn))
                                .Replace("[$EXTEND$]", hiTable.TableType == TableType.Var && hiColumn.IsPrimary ? "primary key" : "")
                                ;
                            break;
                    ...
                    throw new Exception($"字段类型[{dbConfig.DbMapping[hiColumn.FieldType].ToString()} 在Sqlite中没有配置字段模版");
            }
            else
                throw new Exception($"字段[{hiColumn.FieldName}] 对应的字段类型在Sqlite中没有做实现,帮该库不支持该类型");

            return _str_temp_field;
        }


        private DataTable ConvertTableDefinitionFromPRAGMA(DataTable dt_temp, string tableName, bool isTable)
        {

            DataTable dt = new DataTable() { };
            dt.Columns.Add(new DataColumn("TabType", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("TabName", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("FieldNo", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("FieldName", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("IsIdentity", Type.GetType("System.Boolean")));
            dt.Columns.Add(new DataColumn("IsPrimary", Type.GetType("System.Boolean")));
            dt.Columns.Add(new DataColumn("FieldType", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("UseBytes", Type.GetType("System.Int32")));
            dt.Columns.Add(new DataColumn("Lens", Type.GetType("System.Int32")));
            dt.Columns.Add(new DataColumn("PointDec", Type.GetType("System.Int32")));
            dt.Columns.Add(new DataColumn("IsNull", Type.GetType("System.Boolean")));
            dt.Columns.Add(new DataColumn("DbDefault", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("FieldDesc", Type.GetType("System.String")));
            foreach (DataRow item in dt_temp.Rows)
            {
                ...
                {
                    type = dict["FieldType"];
                    lens = int.Parse(dict["Lens"]);
                    pointDec = string.IsNullOrWhiteSpace(dict["PointDec"].ToString()) ? 0 : int.Parse(dict["PointDec"].ToString());
                }

                dt.Rows.Add(isTable ? "Table" : "View", tableName, item["cid"].ToString(), item["name"].ToString(), "False", "0".Equals(item["pk"].ToString()) ? "False" : "True"
                    , type, 4, lens, pointDec, "0".Equals(item["notnull"].ToString()) ? "True" : "False", item["dflt_value"].ToString(), item["name"].ToString());
            }

            return dt;
        }

        private DataTable ConvertIndexDetailFromPRAGMA(DataTable dt_temp, string tableName, string indexName, bool IsPrimary)
        {
            ...
            dt.Columns.Add(new DataColumn("ColumnID", Type.GetType("System.Int32")));
            dt.Columns.Add(new DataColumn("ColumnName", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("Sort", Type.GetType("System.Int32")));
            dt.Columns.Add(new DataColumn("IsPrimary", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("IsUnique", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("Ignore_dup_key", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("Disabled", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("Fill_factor", Type.GetType("System.String")));
            dt.Columns.Add(new DataColumn("Padded", Type.GetType("System.String")));
            ...
            foreach (DataRow item in dt_temp.Rows)
            {
                ...
                //dt.Rows.Add(1==1 ? "Table" : "View", tableName, item["cid"].ToString(), item["name"].ToString(), "False", "0".Equals(item["pk"].ToString()) ? "False" : "True"
                //    , type, 4, lens, pointDec, "0".Equals(item["notnull"].ToString()) ? "True" : "False", item["dflt_value"].ToString(), item["name"].ToString());
            }
        ...
        {
            //table.Columns.Contains("TabType") && table.Columns.Contains("TabName") && table.Columns.Contains("FieldNo") && table.Columns.Contains("FieldName")
            //     && table.Columns.Contains("IsIdentity") && table.Columns.Contains("IsPrimary") && table.Columns.Contains("FieldType")
            //     && table.Columns.Contains("UseBytes") && table.Columns.Contains("Lens") && table.Columns.Contains("PointDec") && table.Columns.Contains("IsNull")
            //     && table.Columns.Contains("DbDefault") && table.Columns.Contains("FieldDesc")
            //     && table.Rows.Count >
        ...
        public DataTable GetTableList(string tabname, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            string _tempsql = dbConfig.Get_TablesPaging;
        ...
        public string BuildTabCreateSql(HiTable hiTable, List<HiColumn> lstHiTable, bool isdrop = false)
        {
            ...
            if (hiTable != null)
            {
                ...
                if (hiTable.TableType == TableType.Var || hiTable.TableType == TableType.Entity
                    || hiTable.TableType == TableType.Local || hiTable.TableType == TableType.Global
                    )
                {
        ...
        public string BuildJoinSql(List<TableDefinition> TableList, Dictionary<string, TabInfo> dictabinfo, List<FieldDefinition> Fields, List<JoinDefinition> Joins, bool issubquery = false)
        {
            ...
            foreach (JoinDefinition joinDefinition in Joins)
            {
                ...
                {
                    throw new HiSqlException($"暂不支持 Right Join");
                }
        ...
        {
            //var hiColumns = DataConvert.CloneHiColumnList(hiColumnsin);
            TabInfo tabInfo = GetTabStruct(tabname);
            string temp_table = $"_sqliteexpert_temp_table_{DateTime.Now.ToFileTime().ToString()}";
            StringBuilder _sql = new StringBuilder(dbConfig.GetModiTableSAVEPOINT);
            ...
            foreach (var hiColumn in tabInfo.Columns)
            {
                ...
                {
                    hiColumn.IsPrimary = false;
                }
                if (hiColumns.Any(t => t.FieldName == hiColumn.FieldName && t.IsPrimary))
                {
        ...
        {
            string _sql = dbConfig.Re_Table.Replace("[$TabName$]", $"{dbConfig.Table_Pre}{tabname}{dbConfig.Table_After}").Replace("[$ReTabName$]", $"{newtabname}");
