...
namespace HiSql
{
    ...
    /// <summary>
    /// Sqlite 无实体ORM SQL编译实现
    /// </summary>
    public class SqliteQuery : QueryProvider
    {
        /*
         将查询进行分块
        1.select [field]
        ...
        IDbConfig dbConfig = new SqliteConfig();
        ...
        void checkData()
        {
            ...
            int _idx2 = 0;
            bool _flag = false;
            SqliteDM sqlServerDM = null;
            sqlServerDM = (SqliteDM)Instance.CreateInstance<SqliteDM>($"{Constants.NameSpace}.{this.Context.CurrentConnectionConfig.DbType.ToString()}{DbInterFace.DM.ToString()}");
            //IDMInitalize dMInitalize = new SqliteDM();
            ...
            if (!this.IsMultiSubQuery)
            {
                ...
                //if (this.WithLockMode != LockMode.NONE) //pengxy 不支持 withlock 
                //{
