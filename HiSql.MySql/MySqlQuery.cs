...
namespace HiSql
{
    public class MySqlQuery : QueryProvider
    {
        /*
         将查询进行分块
        1.select [field]
        ...
        IDbConfig dbConfig = new MySqlConfig();
        public override IDbConfig DbConfig
        {
            get { return dbConfig; }
        }

        public MySqlQuery() : base()
        {
        ...
        void checkData()
        {
            ...
            if (this.IsDistinct)
            {
                sb_distinct = new StringBuilder().Append(mysqlDM.BuilderDistinct());
            }
            else
