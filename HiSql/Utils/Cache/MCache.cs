...
namespace HiSql
{
    ...
    public class MCache : ICache
    {
        ...
        private static readonly object lockHstObj = new object();
        ...
        public bool UnLock(string key)
        {
            ...
            if (GetCache<LckInfo>(key) != null)
            {
                System.Threading.Monitor.Exit(GetCache<LckInfo>(key));
                RemoveCache(key);
        ...
        public List<LckInfo> GetCurrLockInfo()
        {
            ...
            if (globalHSet.ContainsKey(_lockhashname))
            {
                List<string> lckInfosWaitRemove = new List <string>();
               var list = (Hashtable)globalHSet[_lockhashname];
        ...
        {
            List<LckInfo> lckInfos = new List<LckInfo>();
            if (globalHSet.ContainsKey(_lockhishashname))
            {
                var list = (Hashtable)globalHSet[_lockhishashname];
                foreach (string key in list.Keys)
                {
                    if (list[key] != null)
                    {
