    ...
    public class DbFirst : IDbFirst
    {
        ...
        IDM buildIDM(DBType dBType)
        {
            lock (_obkey)
            {
                ...
                {
                    Idm = (IDM)
                        Instance.CreateInstance<IDM>(
                            $"{Constants.NameSpace}.{dBType.ToString()}{DbInterFace.DM.ToString()}"
                        );
        ...
        Tuple<bool, string, string> addColumn(
            IDM idm,
            TabInfo tabInfo,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        {
            ...
            if (
                tabInfo.Columns.Any(c =>
                    c.FieldName.Equals(hiColumn.FieldName, StringComparison.OrdinalIgnoreCase)
                )
            )
            {
                _msg =
                    $"向表[{tabInfo.TabModel.TabName}]添加新字段[{hiColumn.FieldName}]失败 原因:该字段[{hiColumn.FieldName}]已经存在于表[{tabInfo.TabModel.TabName}]中";
            }
            else
            {
                _sql = idm.BuildChangeFieldStatement(
                    tabInfo.TabModel,
                    hiColumn,
                    TabFieldAction.ADD
                );
        ...
        /// <returns></returns>
        public Tuple<bool, string, string> AddColumn(
            string tabname,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        {
            ...
            //获取当前最新物理表结构信息
            HiSqlCommProvider.RemoveTabInfoCache(
                tabname,
                _sqlClient.Context.CurrentConnectionConfig
            );
            //获取最新
        ...
        public Tuple<bool, string, string> CreatePrimaryKey(
            string tabname,
            List<HiColumn> columns,
            OpLevel opLevel
        )
        {
            bool _isok = false;
            ...
            if (
                lstindex.Any(i =>
                    string.Equals(i.IndexType, "Key_Index", StringComparison.OrdinalIgnoreCase)
                )
            )
        ...
        /// <returns></returns>
        public Tuple<bool, string, string> CreateIndex(
            string tabname,
            string indexname,
            List<HiColumn> columns,
            OpLevel opLevel
        ...
        public bool CreateTable(TabInfo tabInfo)
        {
            if (_sqlClient != null)
            {
                ...
                {
                    string _key = Constants
                        .LockTablePre.Replace("[$TabName$]", tabInfo.TabModel.TabName)
                        .Replace("[$DbType$]", _sqlClient.CurrentConnectionConfig.DbType.ToString())
                        .Replace("[$DbServer$]", _sqlClient.CurrentConnectionConfig.DbServer);
                    ...
                    else
                    {
                        ...
                        var rtnexe = Lock.LockOnExecute(
                            ...
                            new LckInfo
                            {
                                UName = _sqlClient.CurrentConnectionConfig.User,
                                Ip = Tool.Net.GetLocalIPAddress()
                            }
                        );
        ...
        /// <returns></returns>
        public Tuple<bool, string, string> CreateView(
            string viewname,
            string viewsql,
            OpLevel opLevel
        )
        ...
        public Tuple<bool, string, string> ModiView(
            string viewname,
            string viewsql,
            OpLevel opLevel
        )
        ...
        Tuple<bool, string, string> delColumn(
            IDM idm,
            TabInfo tabInfo,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        {
            ...
            {
                _msg =
                    $"向表[{tabInfo.TabModel.TabName}]删除字段[{hiColumn.FieldName}]失败 原因:该字段[{hiColumn.FieldName}]不存在于表[{tabInfo.TabModel.TabName}]中";
            }
            else
            {
                _sql = idm.BuildChangeFieldStatement(
        ...
        /// <param name="opLevel"></param>
        /// <returns></returns>
        public Tuple<bool, string, string> DelColumn(
            string tabname,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        ...
        public Tuple<bool, string, string> DelPrimaryKey(string tabname, OpLevel opLevel)
        {
            ...
            if (
                !lst.Any(t =>
                    string.Equals(t.IndexType, "Key_Index", StringComparison.OrdinalIgnoreCase)
                )
            )
            {
                _msg = $"表[{tabname}]没有主键";
            ...
            string primaryKeyName = lst.FirstOrDefault(t =>
                string.Equals(t.IndexType, "Key_Index", StringComparison.OrdinalIgnoreCase)
            ).IndexName;
        ...
        /// <param name="indexname"></param>
        /// <returns></returns>
        public Tuple<bool, string, string> DelIndex(
            string tabname,
            string indexname,
            OpLevel opLevel
        )
        {
            ...
            if (
                lst.Any(t =>
                    t.IndexName.ToLower() == indexname.ToLower() && t.IndexType == "Key_Index"
                )
            )
            {
        ...
        public bool DropTable(string tabname, bool nolog = false)
        {
            if (_sqlClient != null)
            {
                ...
                if (_table.TableType == TableType.Entity)
                {
                    ...
                    _sqlClient
                        .Delete(
                            Constants.HiSysTable["Hi_TabModel"].ToString(),
                            new
                            {
                                TabName = tabname,
                                DbServer = "",
                        ...
                        .ExecCommand();
                    _sqlClient
                        .Delete(Constants.HiSysTable["Hi_FieldModel"].ToString())
                        .Where($"TabName='{tabname.ToSqlInject()}' and DbServer='' and DbName='' ")
                        .ExecCommand();

                    HiSqlCommProvider.RemoveTabInfoCache(
                        tabname,
                        _sqlClient.CurrentConnectionConfig
                    );
                    return v > 0;
                }
                else
        ...
        public List<TableInfo> GetAllTables()
        {
            ...
            DataTable dataTable = _sqlClient
                .Query("Hi_TabModel")
                .Field("*")
                .Where(new Filter { { "TabName", OperType.IN, lst } })
                .Sort("TabName asc")
                .ToTable();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                var _dr = dataTable
                    .AsEnumerable()
                    .Where(p => p.Field<string>("TabName") == tableInfo.TabName)
                    .FirstOrDefault();
                if (_dr != null)
                {
        ...
        Tuple<bool, string, string> modiColumn(
            IDM idm,
            TabInfo tabInfo,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        {
            ...
            string _msg = "";
            string _sql = "";
            var field = tabInfo.Columns.FirstOrDefault(c =>
                c.FieldName.ToLower() == hiColumn.FieldName.ToLower()
            );
            if (field == null)
            {
                _msg =
                    $"向表[{tabInfo.TabModel.TabName}]修改字段[{hiColumn.FieldName}]失败 原因:该字段[{hiColumn.FieldName}]不存在于表[{tabInfo.TabModel.TabName}]中";
            }
            else
            {
                if (
                    field.FieldType != hiColumn.FieldType
                    && !Constants
                        .HiTypeAllowConvertDefinition[field.FieldType]
                        .Contains(hiColumn.FieldType)
                )
                {
                    _isok = false;
                    _msg =
                        $"表【{hiColumn.TabName}】的字段【{hiColumn.FieldName}】类型【{field.FieldType.ToString()}】不允许转换成【{hiColumn.FieldType.ToString()}】";
                    return new Tuple<bool, string, string>(_isok, _msg, _sql);
                }
                _sql = idm.BuildChangeFieldStatement(
                    tabInfo.TabModel,
        ...
        /// <param name="opLevel"></param>
        /// <returns></returns>
        public Tuple<bool, string, string> ModiColumn(
            string tabname,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        ...
        /// <param name="opLevel"></param>
        /// <returns></returns>
        Tuple<bool, string, string> reColumn(
            IDM idm,
            TabInfo tabInfo,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        {
            ...
            if (
                hiColumn.FieldName.ToLower() != hiColumn.ReFieldName.ToLower()
                && !string.IsNullOrEmpty(hiColumn.ReFieldName)
            )
            {
                if (
                    !tabInfo.Columns.Any(c => c.FieldName.ToLower() == hiColumn.FieldName.ToLower())
                )
                {
                    _msg =
                        $"向表[{tabInfo.TabModel.TabName}]-[{hiColumn.FieldName}]重命名为字段[{hiColumn.ReFieldName}]失败 原因:该字段[{hiColumn.FieldName}]不存在于表[{tabInfo.TabModel.TabName}]中";
                }
                else
                {
                    //重命名字段不在表中才可以进行重命名
                    if (
                        !tabInfo.Columns.Any(c =>
                            c.FieldName.ToLower() == hiColumn.ReFieldName.ToLower()
                        )
                    )
                    {
                        hiColumn.FieldDesc = hiColumn.FieldDesc.IsNullOrEmpty()
                            ? hiColumn.FieldName
                            : hiColumn.FieldDesc;
                        _sql = idm.BuildChangeFieldStatement(
                            tabInfo.TabModel,
                            hiColumn,
                            TabFieldAction.RENAME
                        );
                        ...
                        if (opLevel == OpLevel.Execute)
                        {
                            ...
                            try
                            {
                                ...
                                _isok = true;
                                _msg =
                                    $"向表[{tabInfo.TabModel.TabName}]-[{hiColumn.FieldName}]重命名为字段[{hiColumn.ReFieldName}]成功";
                            }
                            catch (Exception E)
                            {
                                _sqlClient.RollBackTran();
                        ...
                        {
                            _msg =
                                $"向表[{tabInfo.TabModel.TabName}]-[{hiColumn.FieldName}]重命名为字段[{hiColumn.ReFieldName}]检测成功";
                        }
                    }
                    else
                    {
                        _msg =
                            $"向表[{tabInfo.TabModel.TabName}]-[{hiColumn.FieldName}]重命名为字段[{hiColumn.ReFieldName}]失败 原因:该字段[{hiColumn.ReFieldName}] 已经存在";
                    }
                }
            }
            else
        ...
        /// <param name="opLevel"></param>
        /// <returns></returns>
        public Tuple<bool, string, string> ReColumn(
            string tabname,
            HiColumn hiColumn,
            OpLevel opLevel
        )
        {
            ...
            {
                _sqlClient
                    .Update(Constants.HiSysTable["Hi_FieldModel"], hiColumn)
                    .OnlyWhere(
                        $"TabName='{hiColumn.TabName}' and FieldName='{_col.FieldName}' and DbServer='{hiColumn.DbServer}' and DbName='{hiColumn.DbName}'"
                    )
                    .ExecCommand();
                HiSqlCommProvider.RemoveTabInfoCache(
                    tabname,
        ...
        /// <param name="opLevel"></param>
        /// <returns></returns>
        public Tuple<bool, string, string> ModiTable(
            TabInfo tabInfo,
            OpLevel opLevel,
            bool onlychangetable = false
        )
        {
            ...
            //获取当前最新物理表结构信息
            HiSqlCommProvider.RemoveTabInfoCache(
                tabInfo.TabModel.TabName,
                _sqlClient.Context.CurrentConnectionConfig
            );
            //获取最新
            TabInfo tabinfo = idm.GetTabStruct(tabInfo.TabModel.TabName);
            List<FieldChange> fieldChanges = HiSqlCommProvider.TabToCompare(
                tabInfo,
                tab,
                _sqlClient.Context.CurrentConnectionConfig.DbType
            );

            fieldChanges = fieldChanges
                .Where(f =>
                    f.ChangeDetail.Where(c =>
                        c.AttrName.Equals("DBDefault", StringComparison.OrdinalIgnoreCase)
                        && c.ValueA.Equals("EMPTY", StringComparison.OrdinalIgnoreCase)
                        && c.ValueB.Equals("NONE", StringComparison.OrdinalIgnoreCase)
                    )
                        .ToList()
                        .Count == 0
                )
            ...
            if (
                tab.PrimaryKey.Count == tabInfo.PrimaryKey.Count
                && tab.PrimaryKey.Select(t => t.FieldName)
                    .ToList()
                    .All(tabInfo.PrimaryKey.Select(t => t.FieldName).ToList().Contains)
            )
            {
                bool _haskey = false;
            ...
            if (
                _sqlClient.Context.CurrentConnectionConfig.DbType == DBType.Sqlite
                && changes.Count() > 0
            )
            {
            ...
            else
            {
                foreach (FieldChange field in changes)
                {
                    ...
                    var hicol = tabInfo
                        .Columns.Where(c => c.FieldName.ToLower().Equals(field.FieldName.ToLower()))
                        .FirstOrDefault();
                    if (field.IsTabChange)
                    {
                        ...
                        else
                        {
                            if (field.Action == TabFieldAction.DELETE)
                            {
                                var _hicol = tab
                                    .Columns.Where(c =>
                                        c.FieldName.ToLower().Equals(field.FieldName.ToLower())
                                    )
                                    .FirstOrDefault();
                                if (_hicol != null)
                                {
                ...
                if (reBuilderPrimaryKey)
                {
                    ...
                    )
                    {
                        ...
                        string delPrimaryKey = idm.DropIndex(
                            tabInfo.TabModel.TabName,
                            primaryKeyName,
                            true
                        );
                    ...
                    {
                        string createPrimaryKey = idm.CreatePrimaryKey(
                            tabInfo.TabModel.TabName,
                            tabInfo.PrimaryKey
                        );
                ...
                if (_isok)
                {
                    if (opLevel == OpLevel.Execute)
                    {
                        ...
                        try
                        {
                            ...
                            if (lstdel.Count > 0)
                            {
                                del_count = _sqlClient
                                    .Delete(
                                        Constants.HiSysTable["Hi_FieldModel"].ToString(),
                                        lstdel
                                    )
                                    .ExecCommand();
                            }
                            ...
                            if (lstchg.Count > 0)
                            {
                                ...
                                if (
                                    tabInfo.TabModel.TabName.Equals(
                                        Constants.HiSysTable["Hi_FieldModel"].ToString(),
                                        StringComparison.OrdinalIgnoreCase
                                    )
                                )
        ...
        /// <param name="opLevel"></param>
        /// <returns></returns>
        public Tuple<bool, string, string> ReTable(
            string tabname,
            string newtabname,
            OpLevel opLevel
        )
                ...
                if (opLevel == OpLevel.Execute)
                {
                    ...
                    try
                    {
                        ...
                        _sqlClient
                            .Delete("Hi_TabModel")
                            .Where(
                                $"TabName='{tabname.ToSqlInject()}' and DbServer='' and DbName=''"
                            )
                            .ExecCommand();
                        _sqlClient
                            .Delete("Hi_FieldModel")
                            .Where(
                                $"TabName='{tabname.ToSqlInject()}' and DbServer='' and DbName=''"
                            )
                            .ExecCommand();

                        _sqlClient.Context.DBO.ExecCommand(
                            idm.BuildSqlCodeBlock(
                                idm.BuildTabStructSql(_tabinfo.TabModel, _tabinfo.GetColumns)
                            )
                        );
