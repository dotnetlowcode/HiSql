...
namespace HiSql
{
    ...
    /// </summary>
    public class HanaQuery : QueryProvider
    {
        /*
         将查询进行分块
        1.select [field]
        ...
        //            if (dbFunction.IsIn<DbFunction>(DbFunction.AVG, DbFunction.SUM, DbFunction.MIN, DbFunction.MAX))
        ...
        //                this.Ranks.Add($"rank() over( order by {dbFunction.ToString()}({field}) {sortType.ToString()}) as {dbConfig.Field_Pre}{asname}{dbConfig.Field_After}");
        //            else if (dbFunction.IsIn<DbFunction>(DbFunction.NONE))
        //            {
        //                this.Ranks.Add($"rank() over( order by  {field} {sortType.ToString()}) as {dbConfig.Field_Pre}{asname}{dbConfig.Field_After}");
        //            }
        //            else
        //                throw new Exception($"{rank.ToString()} 不支持[{dbFunction.ToString()}]此函数");
        //            break;
        ...
        void checkData()
        {
            ...
            if (!this.IsMultiSubQuery)
            {
                ...
                if (this.TableList.Count > 0)
                {
                    ...
                    foreach (TableDefinition table in this.TableList)
                    {
                        ...
                        {
                            //全局缓存
                            tabinfo = hanaDM.GetTabStruct(table.TabName);
                        }
            ...
            Tuple<string, string,List<HiColumn>> queryresult = hanaDM.BuildQueryFieldSql(dictabinfo, (QueryProvider)this);
            ...
            //检测JOIN关联条件字段
            sb_join.Append(hanaDM.BuildJoinSql(this.TableList, dictabinfo, this.Fields, this.Joins));
            ...
            //排序字段
            sb_sort.Append(hanaDM.BuildOrderBySql(ref sb_group, dictabinfo, (QueryProvider)this));
