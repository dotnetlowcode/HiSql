    ...
    public class HanaUpdate: UpdateProvider
    {
        ...
        void checkData()
        {
            ...
            if (this.Data.Count > 0)
            {
                ...
                {
                    //Tuple<Dictionary<string, string>, Dictionary<string, string>> result = this.CheckData(_isdic, this.Table, obj, Context.DMInitalize, tabinfo, _field, _isonly);
                    ...
                    if (this.Data.Count > 1)
                    {
                        sb.AppendLine($"    EXEC '{Context.DMTab.BuildUpdateSql(tabinfo, this.Table, rtn_check.Item1[_idx], rtn_check.Item2[_idx], sql_where).Replace("'", "''")}';");
                    }
                    else
                    {
